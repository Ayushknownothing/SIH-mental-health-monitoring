...
4. Creates the interaction in Supabase.
5. Stores the emotion result in Supabase.
6. Returns the interaction and emotion result to the client.
...
Conversation history does not replace Model 2.
...
Text → Backend → History + AI → Emotion → Llama → Backend → Supabase    ✓

Voice → Backend → History + Whisper → Emotion → Llama → Backend → Supabase    ✓
